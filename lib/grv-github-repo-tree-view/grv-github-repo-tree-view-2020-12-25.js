...
GRV.urlViewer = "";
GRV.urlApi = "https://api.github.com/repos/pushme-pullyou/tootoo-2020/git/trees/master?recursive=1";
GRV.urlSource = "https://github.com/pushme-pullyou/tootoo-2020/";
GRV.path = "";

GRV.iconOctoCat = `<img width=14 src="${ GRV.path }lib/icons/octicon.svg">`;
GRV.link = `<img width=14 src="${ GRV.path }lib/icons/icon-link-external.svg">`;
	...
	const htm = `
			<details open>
				...
				<summary class="summary-primary gmd-1" title="View selected items">repository files
					...
					<span class="info">
						<img class=infoImg src="${ GRV.path}lib/icons/noun_Information_585560.svg">
...
GRV.getFilesSimple = function ( subtree, files ) {
	...
	const filtered = files
		.filter( file => file.slice( 0, file.lastIndexOf( "/" ) ) === str )
		.filter( file => file.endsWith( ".md" ) || file.endsWith( ".jpg" ) )
		.map( item => `

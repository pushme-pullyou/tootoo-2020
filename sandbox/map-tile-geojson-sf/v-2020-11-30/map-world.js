...
MAP.getTilesBitmaps = function () {
	...
	MAP.latDelta = latDelta = ( centerLatMax - centerLatMin );
	MAP.lonDelta = lonDelta = centerLonMax - centerLonMin;

	console.log( "", lonDelta );

	MAP.centerLatCen = centerLatMin + 0.5 * ( centerLatMax - centerLatMin );
	MAP.centerLonCen = centerLonMin + 0.5 * ( centerLonMax - centerLonMin );

	MAP.latMin = MAP.centerLatCen - 0.5 * MAP.rows * ( centerLatMax - centerLatMin );
	MAP.lonMin = MAP.centerLonCen - 0.5 * MAP.cols * ( centerLonMax - centerLonMin );

	MAP.latMax = MAP.centerLatCen + 0.5 * MAP.rows * ( centerLatMax - centerLatMin );
	MAP.lonMax = MAP.centerLonCen + 0.5 * MAP.cols * ( centerLonMax - centerLonMin );

	const latOffsetTarget = Math.abs( centerLatMax - MAP.latitude );
	const lonOffsetTarget = Math.abs( centerLonMax - MAP.longitude );

	const latDiff = latOffsetTarget / Math.abs( latDelta);
	const lonDiff = lonOffsetTarget / Math.abs( lonDelta );

	MAP.tellTaleLatLon();

	MAP.tellTaleLatLon( MAP.latMax, MAP.lonMax );

	MAP.tellTaleLatLon( 37.796, -122.398 );
	...
	MAP.stats = `
	<p>
		columns: ${ MAP.cols } ~ rows: ${ MAP.rows }<br>
		centerLatCen ${ MAP.centerLatCen }<br>
		centerLonCen ${ MAP.centerLonCen }<br>
		latMin ${ MAP.latMin }<br>
		lonMin ${ MAP.lonMin }<br>
		latMax ${ MAP.latMax }<br>
		lonMax ${ MAP.lonMax }<br>
	</p>
	<p>
...
MAP.tellTaleLatLon = function ( lat = MAP.centerLatCen, lon = MAP.centerLonCen ) {

	const tt = THRU.addTellTale();
	//const latMin = centerLatMax - 2.5 * latDelta;
	//const lonMin = centerLonMax - 2.5 * lonDelta;
	const x = MAP.unitsPerTile * ( MAP.centerLonCen  - lon ) / MAP.lonDelta;
	const y = MAP.unitsPerTile * ( MAP.centerLatCen - lat  ) / MAP.lonDelta;
	tt.position.set( x, y, 0 );
	tt.scale.z = 500;
	tt.castShadow = true;
